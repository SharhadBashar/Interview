...
function shipReport(){
  var ships = allShips();
  ships.splice(1, ships.length - 2);
  return ships;
}

function howDangerous(coordinates){
  var cellContent = lightCell(coordinates);
  if (cellContent === '~'){
    return 50;
  }
  else if (cellContent === '^'){
...
function percentageReport(){
  var percentages = [];
  var gridSize = parseFloat(totalCells());
  var rocks = parseFloat(allRocks().length);
  var currents = parseFloat(allCurrents().length);
  percentages.push((rocks/gridSize*100).toFixed(2));
  percentages.push((currents/gridSize*100).toFixed(2));
  return percentages;
}
...
function calcDistance(cord_1, cord_2){
  let column_1 = convertColumn(cord_1);
  let row_1 = Number(cord_1.substring(1)) - 1;
  let column_2 = convertColumn(cord_2);
  let row_2 = Number(cord_2.substring(1)) - 1;
  let horizontal = Math.abs(column_1 - column_2);
  let vertical = Math.abs(row_1 - row_2);
  let distance = Math.sqrt(Math.pow(horizontal, 2) + Math.pow(vertical, 2));
